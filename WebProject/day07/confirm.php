...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if editing (update) or inserting (new)
    if (isset($_POST['id']) && !empty($_POST['id'])) {
        $id = $_POST['id'];

        // Update existing student
        $sql = "UPDATE users SET studentName = ?, birthday = ?, subject = ?, course = ?" .
                (isset($avatar_name) ? ", avatar = ?" : "") . " WHERE id = ?";
        $stmt = $conn->prepare($sql);
        
        // Bind param conditionally based on avatar presence
        if (isset($avatar_name)) {
            $stmt->bind_Param('sssssi', $studentName, $dob, $subject, $course, $avatar_folder, $id);
        } else {
            $stmt->bind_Param('ssssi', $studentName, $dob, $subject, $course, $id);
        }
    ...
    $sql = "INSERT INTO users (studentName, birthday, subject, course, avatar) VALUES (?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('sssss', $studentName, $birthday, $subject, $course, $avatar_folder);
    ...
    <form>
        <div style="border: gainsboro 1px solid; padding: 15px; width: 600px; box-shadow: 1px 1px 1px 2px gainsboro; border-radius: 5px; background-color: white">
            ...
            <div class="button-container">
                ...
                <button type="button" id="btn-confirm">Xác nhận</button>
            </div>
        </div>
    </form>
    <script>
