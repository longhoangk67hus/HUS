...
```{r}
so_sinh_vien <- 70
diem <- seq(0, 10, by = 0.5)
bo_du_lieu <- sample(diem, so_sinh_vien, replace = TRUE)
print(bo_du_lieu)
...
```{r}
phan_loai_diem <- cut(bo_du_lieu, breaks = c(0, 4, 6, 8, 10), labels = c("D", "C", "B", "A"))
du_lieu <- data.frame(Diem = bo_du_lieu, PhanLoai = phan_loai_diem)
print(du_lieu)
...
```{r}
sum(work_time > 20)
sum(work_time <= 17) / length(work_time) * 100
```
...
```{r}
oldMean = mean(work_time)
work_time[11:15] = c(22,23,18,25,30)
newMean = mean(work_time)
if (oldMean < newMean) {
...
```{r}
data = read.csv("/Users/ngoclong7204/Documents/Workspace/Introduction to data analys/MidTerm/data.csv")
View(data)
attach(data)
```
...
```{r}
nonSmokerHeight = data[data$Smoke == "Never",]$Height
nonSmokerHeight
```
...
```{r}
averageRight = mean(data[data$Sex == "Female" & data$W.Hnd == "Right",]$Height)
print(paste("Chieu cao trung binh cua nu la",averageRight))
```
...
```{r}
so_sanh <- function() {
  people = data[data$Age < 25 & data$Smoke == "Heavy",]
  female_pulse = mean(people[people$Sex == "Female",]$Pulse)
  male_pulse = mean(people[people$Sex == "Male",]$Pulse)
  if (female_pulse > male_pulse) {
    print("Nhip tim trung binh cua nu cao hon cua nam")
  }
  if (female_pulse < male_pulse) {
    print("Nhip tim trung binh cua nu thap hon cua nam")
  }
  if (female_pulse == male_pulse) {
    print("Nhip tim trung binh cua nu bang cua nam")
  }
}
so_sanh()
```
...
```{r}
exercise <- function() {
  exercise_result = data[data$Age < 30 & data$Smoke == "Never",]
  min_pulse = min(exercise_result$Pulse)
  result_exercise = exercise_result[exercise_result$Pulse == min_pulse,]$Exer
  print(paste("cuong do tap the duc cua nguoi tuoi 30 la",result_exercise))
  print(paste("nhip tim do bang",min_pulse))
}
exercise()
